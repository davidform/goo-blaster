...
with sync_playwright() as pw:
 ...
 assert p.locator('#gardenQuestDetail').inner_text()=='Use 3 seeds. Clear 1 stage(s) to harvest 3 petals.'
 # Four farming cycles, unchanged first repair cost 12. Grow through the real settlement route.
 for cycle in range(4):
  ...
  p.locator('#gardenQuestAction').click();assert p.locator('#btnPlay').is_visible()
  p.locator('#btnPlay').click();p.wait_for_function('G&&G.running')
  p.evaluate('endGame(true)');p.locator('#btnGarden').click()
 ...
 p.locator('#gardenQuestCrop').select_option('1');assert 'Clear 2' in p.locator('#gardenQuestDetail').inner_text()
...
print('PASS first restoration through task board; real settlement; choice/feedback; save; caps; 132 state/locale/size checks')
