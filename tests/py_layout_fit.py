...
with sync_playwright() as pw:
    ...
    print("\n=== 4. 開場操作提示（hintMove/hintDash/hintHearts/hintChest）不能超出畫面 ===")
    # v0.9.31 新增：這四行提示原本用固定字級、從來沒被本檔測過——前三段測試
    # 都在 start() 之後立刻把 G.hasMoved 設成 true，等於刻意跳過這個只在
    # 「玩家還沒移動過」時才會畫的區塊。改 hintMove 文字時才發現：好幾種語言
    # （de/fr/es/pt-BR/ru）本來就超出畫面寬度最多 30% 以上，只是沒人測過。
    # 已改成用 fitFont 動態縮字，這裡把它釘住，往後不會再回歸。
    GET_HINT_WANT="()=>['hintMove','hintDash','hintHearts','hintChest'].map(k=>T(k))"
    for code in langs:
        pg.evaluate("""(code)=>{
            applyLanguage(code);
            LV_IDX=0; start(); G.hasMoved=false; G.t=0;
            window.__box.length=0;
        }""",code)
        want=pg.evaluate(GET_HINT_WANT)
        boxes=[]
        for _ in range(20):
            pg.wait_for_timeout(60)
            boxes+=pg.evaluate("()=>{const b=window.__box.slice(); window.__box.length=0; return b;}")
            if all(any(bx["t"]==w for bx in boxes) for w in want):
                break
        over=[bx for bx in boxes if bx["t"] in want and (bx["left"]<-1 or bx["right"]>W+1)]
        uniq={}
        for bx in over: uniq[bx["t"]]=bx
        over=list(uniq.values())
        ck(f"[{code}] 四行開場提示全部沒有超出畫面", not over,
           [(x["t"][:40],round(x["left"]),round(x["right"])) for x in over])
        # 開場提示只在還沒動過時顯示，測完立刻讓角色動過，避免影響後面的批次
        pg.evaluate("()=>{ if(G) G.hasMoved=true; }")
