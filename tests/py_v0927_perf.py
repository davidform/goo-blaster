...
同批次交錯 A/B，不同時開兩局互搶繪圖資源。AB/BA/BA/AB 平衡先後順序。
保留對照 >=20 FPS、同伴損失 <20%、單局 >=30 FPS 三個門檻。
"""
import asyncio, http.server, socketserver, threading, functools, sys, statistics, os, json, hashlib
from pathlib import Path
from test_paths import BROWSER_CHANNEL, GAME_ROOT, ARTIFACTS
...
WORST = """
([withAlly])=>{
  let seed=927;Math.random=()=>{seed=(Math.imul(seed,1664525)+1013904223)>>>0;return seed/4294967296;};
...
async def one(browser, with_ally):
    assert not browser.contexts, 'FPS sampler must not compete with another game context'
    ...
    await (await c.new_cdp_session(pg)).send('Emulation.setCPUThrottlingRate',{'rate':int(os.getenv('GOO_PERF_CPU','1'))})
    ...
    r=await pg.evaluate("""([f0,t0])=>{ clearInterval(window.__k);
        ...
        return {fps:+fps.toFixed(1), frames:window.__frames-f0, elapsed_ms:performance.now()-t0,
                hidden:document.hidden, enemies:G.E.length, bullets:G.B.length,
...
async def main():
    async with async_playwright() as pw:
        ...
        print("=== 第50關最壞情況：單局交錯 A/B，AB/BA/BA/AB ===")
        off,on=[],[]
        order=[]
        for i,pair in enumerate([(False,True),(True,False),(True,False),(False,True)]):
            for with_ally in pair:
                sample=await one(b,with_ally)
                assert not b.contexts, 'Sampler leaked a browser context'
                (on if with_ally else off).append(sample)
                order.append({'with_ally':with_ally,**sample})
                print(f"  第{i+1}輪 {'有' if with_ally else '無'}同伴 {sample['fps']:.1f} fps（敵{sample['enemies']} 我彈{sample['bullets']} 同伴{sample['ally']}）",flush=True)
        await b.close()
        ...
        ck("對照組沒有同伴",all(x['ally']==0 for x in off))
        ck("頁面可見且量測至少8秒",all(not x['hidden'] and x['elapsed_ms']>=8000 for x in off+on))
        ...
        if f_off < 20:
            print(f"  ⚠ 單局對照只有 {f_off:.1f} fps；可能為遊戲或環境負載，不能由FPS單獨判定原因。")
            ck("環境足夠安靜、量得到效能", False, f"對照組 {f_off:.1f} fps")
        else:
            ck("同伴造成的 FPS 損失 < 20%（同批次相對值）",
               f_on > f_off*0.80, f"{(f_on-f_off)/f_off*100:+.1f}%")
        # Always assess absolute FPS, even when the control lacks measurement headroom.
        ck("單局有同伴時 FPS ≥ 30",f_on>=30,round(f_on,1))
        ...
        (ARTIFACTS/('perf-isolated-cpu'+os.getenv('GOO_PERF_CPU','1')+'.json')).write_text(json.dumps({
            'source_sha256':hashlib.sha256((Path(ROOT)/'index.html').read_bytes()).hexdigest(),
            'test_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
            'order':order,'control_fps':f_off,'ally_fps':f_on,'failures':fails},indent=2),encoding='utf8')
        return 0
