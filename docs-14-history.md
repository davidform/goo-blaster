...
## v0.9.76 — 庭院就地操作與工作頁重排（2026-09-26）
- 根因：田地／小屋操作位於場景下方，gardenSelect以scrollIntoView帶玩家離開所選物件。廚房、委託與裝飾也堆在同一長頁。
- 同條件實測390×844：v75點第一田scrollTop增加477px，種植按鈕y787–831被y769開始的導覽遮住；v76增加0px，按鈕y617–661完整可見。基準3b76d1b；context76-comparison.json保留原數據，不重做v75發布。
- 唯一改動為庭院互動版面：就地popover、作物圖示選擇、聚焦廚房／委託／佈置頁、固定分類列、寬螢幕並排與村莊初始鏡頭。既有11語言文字，無存檔／平衡／倒數改動。
- 使用瀏覽器原生popover的外側／Escape關閉，加上明確返回按鈕；選取外框及可視範圍限制。其他田地成熟時保留所選作物及焦點。
- 補tests/py_garden_context.py：44語言尺寸、無選取捲頁、實際種植、焦點／時間邊界、工作页隔離與離線。既有測試改用可見作物按鈕和首頁主題地標；未降低遊戲機制斷言。
- 初輪131827保留三項FAIL：手機分類按鈕文字擠窄導致田地落在導覽後；任務按鈕超出首屏；倍率說明誤隱藏。修正分類直向圖示及倍率說明，131941三項通過。132252診斷各項通過但途中補焦點保留造成SHA改變，不作最終驗收。
- 教訓：可見元素不代表可操作，必須量測與固定導覽的重疊；重畫倒數也要保護尚未提交的選擇。詳細研究／限制见docs-20-garden-layout.md；完整測試及交付證據後補。

- 132500完整批次因截圖發現繁中back字串實為「回主畫面」而中止，保留紀錄；改沿用11語言cancel，補中英標籤斷言。最終驗收重新由133204批次執行，不拼接中止報告。

- 最終完整批次py_ui_fixes的HUD幾何項初次FAIL：固定300ms後pause仍0×0，MUTE仍resize初始化下緣114，尚未由drawHUD覆寫。改成等待可見非零暫停键後量測；沿用原112px遮擋門檻，遊戲檔未變，完成後同SHA補驗。

- 最終完整133204初輪82PASS／1FAIL；135414 HUD補驗PASS（實際pause y14–58、mute下緣58，原112門檻通過）。validate_reports核對同SHA合併83/83。
- 最終HTML SHA 62af7c7fa665bdb0da759d8d565d83176381ba50746e98f9160c0a91785b72cc。py_test9第1關46秒／第2關59秒通關，第3關85秒陣亡，保留觀察，不改硬門檻或平衡。
